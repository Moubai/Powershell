...
#Variables :)
$pmanagers = @("snmpserver") # ADD YOUR MANAGER(s) in format @("manager1","manager2")
$CommString = @("mysnmpString") # ADD YOUR COMM STRING(s) in format @("Community1","Community2")
$sysContact = @("my@email.com") #contact email
$sysLocation = @("you street address") #emplacement physique
...
If ($check.Installed -eq "True"){
	...
	Foreach ($String in $CommString){
		...
		Foreach ($Location in $sysLocation){
			reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\SNMP\Parameters\RFC1156Agent" /v sysLocation /t REG_SZ /d $Location /f
		}
		Foreach ($contact in $sysContact){
			reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\SNMP\Parameters\RFC1156Agent" /v sysContact /t REG_SZ /d $contact /f
		}
	}
}
Else {
